...
class SpeculativeConfig:
    ...
    final selection is also capped to half of each request/head LRU page target."""

    retrospec_min_draft_tokens: int = Field(default=1, gt=0)
    """Minimum number of tokens generated by one RetroSpec draft block."""
    retrospec_max_draft_tokens: int = Field(default=16, gt=0)
    """Maximum number of tokens generated by one RetroSpec draft block."""

    retrospec_index_update_interval: int = Field(default=1024, gt=0)
    """Number of newly stable generation tokens in one incremental index
    update. This controls both the generation transition boundary and the size
    of each independently clustered incremental segment."""

    retrospec_draft_margin_threshold: float | None = Field(default=None, ge=0)
    """Stop the current draft block when the top-1/top-2 logit margin is
    below this value."""
    retrospec_sparse_margin_threshold: float | None = Field(default=None, ge=0)
    """Trigger expanded verification when the sparse-verification logit margin is
    below this value."""
    retrospec_expanded_margin_threshold: float | None = Field(default=None, ge=0)
    """Trigger full verification when the expanded-verification logit margin is
    below this value."""
    retrospec_hit_attn_threshold: float | None = Field(default=None, ge=0, le=1)
    """Stop the current draft block when exact-hit attention mass is
    below this value."""
    retrospec_retrieval_attn_threshold: float | None = Field(default=None, ge=0, le=1)
    """Trigger expanded verification when retrieval-zone attention mass is
    below this value."""
    retrospec_expanded_attn_threshold: float | None = Field(default=None, ge=0, le=1)
        ...
        if self.method == "retrospec":
            factors.extend(
                [
                    ...
                    self.retrospec_min_draft_tokens,
    ...
    def __post_init__(self):
        ...
        if self.model is None and self.num_speculative_tokens is not None:
            if self.method == "mtp":
                ...
                self.model = "suffix"
            elif self.method == "retrospec":
                self.model = "retrospec"
            else:
                raise ValueError(
    ...
    def _validate_retrospec(self) -> None:
        if self.retrospec_retrieval_ratio + self.retrospec_estimation_ratio > 1:
            raise ValueError(
                "The sum of retrospec_retrieval_ratio and "
                "retrospec_estimation_ratio must not exceed 1"
            )
        # Expanded verification doubles the retrieval area
        # while remaining within retrieval + estimation capacity.
        if self.retrospec_retrieval_ratio > self.retrospec_estimation_ratio:
            raise ValueError(
                "retrospec_estimation_ratio must be greater than or equal to "
                "retrospec_retrieval_ratio"
            )

        if self.retrospec_min_draft_tokens > self.retrospec_max_draft_tokens:
            raise ValueError(
                "retrospec_min_draft_tokens must be less than or equal to "
                "retrospec_max_draft_tokens"
            )
        if self.num_speculative_tokens is None:
            raise ValueError(
                "num_speculative_tokens must be provided when using RetroSpec"
            )
        if self.retrospec_max_draft_tokens > self.num_speculative_tokens:
            raise ValueError(
                "retrospec_max_draft_tokens must be less than or equal to "
    ...
    def __repr__(self) -> str:
        ...
        model = (
            None
            if method in ("ngram", "suffix", "retrospec")
