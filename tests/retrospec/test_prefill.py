...
class TrackingQwen2Model(Qwen2Model):
    def forward_layer(
        self,
        layer_index: int,
        positions: torch.Tensor,
...
def make_qwen2_model(*, start_layer: int = 0, end_layer: int = 2) -> TrackingQwen2Model:
    model = TrackingQwen2Model.__new__(TrackingQwen2Model)
    nn.Module.__init__(model)
    model.start_layer = start_layer
    model.end_layer = end_layer
    model.layers = nn.ModuleList([AddLayer(1.0), AddLayer(2.0), AddLayer(3.0)])
...
def test_qwen2_forward_layer_uses_global_index_and_forwards_kwargs():
    model = make_qwen2_model(start_layer=1, end_layer=3)
    positions = torch.arange(2)
    hidden_states = torch.ones(2, 4)
...
@pytest.mark.parametrize("layer_index", [0, 3])
def test_qwen2_forward_layer_rejects_layer_outside_pipeline_range(layer_index: int):
    model = make_qwen2_model(start_layer=1, end_layer=3)

    with pytest.raises(ValueError, match="not owned by this pipeline rank"):
        model.forward_layer(layer_index, torch.arange(1), torch.ones(1, 4), None)


def test_qwen2_regular_forward_uses_layer_interface_without_changing_output():
    model = make_qwen2_model()
    positions = torch.arange(2)
    inputs_embeds = torch.ones(2, 4)
    pp_group = SimpleNamespace(is_first_rank=True, is_last_rank=True)

    with patch("vllm.model_executor.models.qwen2.get_pp_group", return_value=pp_group):
        output = model(
            input_ids=None,
            positions=positions,
            intermediate_tensors=None,
...
def test_qwen2_finalize_hidden_states_accepts_materialized_residual():
    model = make_qwen2_model()
    hidden_states = torch.ones(2, 4)
    pp_group = SimpleNamespace(is_last_rank=True)
...
def test_resolve_retrospec_layer_model_accepts_inner_and_wrapped_model():
    ...
    qwen2_model = make_qwen2_model()
    wrapper.model = qwen2_model
    assert resolve_retrospec_layer_model(qwen2_model) is qwen2_model
    assert resolve_retrospec_layer_model(wrapper) is qwen2_model
